...
function update() {
    ...
    if (velocityX > 0){
        if (predictedSpotY2 > paddle1.y+paddle1Height/2 && paddle1.y < canvas.height-paddle1Height-paddle1SpeedY) {
            paddle1.y += paddle1SpeedY*movingSpeed*secondsPassed;
        }
        else if (predictedSpotY2 > paddle1.y+paddle1Height/2) {
            paddle1.y = canvas.height-paddle1Height
        }
        if (predictedSpotY2 < paddle1.y+paddle1Height/2 && paddle1.y > 0) {
            paddle1.y -= paddle1SpeedY*movingSpeed*secondsPassed;
        }
        else if (predictedSpotY2 < paddle1.y+paddle1Height/2) {
            paddle1.y = 0
    ...
    if (ball.x > canvas.width-ballSize+posVelocityX) {
        scoreLeft+=1
        ovrMediumLeft+=1
        localStorage.setItem("ovrMediumLeft", ovrMediumLeft);
    ...
    if (ball.x < ballSize-posVelocityX){
        scoreRight+=1
        ovrMediumRight+=1
        localStorage.setItem("ovrMediumRight", ovrMediumRight);
        paddle1Height = paddleHeight
        paddle2Height = paddleHeight
        ballBounciness = setBallBounciness
        paddle1SpeedY = paddleSpeedY
