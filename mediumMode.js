...
function update() {
    ...
    if (ball.x <= paddle1.x+30 && ball.x >= paddle1.x-posVelocityX+25 && ball.y <= paddle1.y+110 && ball.y >= paddle1.y-10){
        ...
        velocityY = negVelocityY
    }//calculate where the ball will be when ball is moving towards paddle or away
    if(velocityX < 0){
        calculate1()
    }
    if(velocityX > 0){
    ...
    if (velocityX > 0 && paddle1.y+50 < predictedSpotY2 && paddle1.y < canvas.height-paddleHeight){
        paddle1.y += paddleSpeedY;
    }
